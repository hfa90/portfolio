// ================================================================
// CONFIGURAÃ‡ÃƒO E NAVEGAÃ‡ÃƒO (SPA)
// ================================================================
...
// ================================================================
// MÃ“DULO 1: DASHBOARD INTELIGENTE
// ================================================================
...
// ================================================================
// 3. FUNÃ‡Ã•ES DE APOIO (Para evitar erros de 'not defined')
// ================================================================
function inicializarGrafico() {
    ...
    window.meuGrafico = new Chart(ctx, {
        type: 'line',
        data: { labels: ['S1', 'S2', 'S3', 'S4'], datasets: [{ label: 'PreÃ§os', data: [10, 15, 8, 12], borderColor: '#2e8b57' }] }
    });
}

// ================================================================
// MÃ“DULO 2: GERENCIAMENTO DE SUPERMERCADOS
// ================================================================
            ...
            container.innerHTML = data.mercados.map(m => `
                <div class="col-md-4">
                    <div class="card p-3 shadow-sm border-0 h-100">
                        ...
                        <small class="text-muted mt-2"><i class="fas fa-map-marker-alt"></i> ${m.endereco || 'Sem endereÃ§o'}</small>
                        <button class="btn btn-sm btn-success mt-3" onclick="abrirGestaoPrecos(${m.id}, '${m.nome}')">
                            <i class="fas fa-tags me-1"></i> Ver PreÃ§os
                        </button>
                    </div>
                </div>
            `).join('');
...
// ================================================================
// MÃ“DULO 3: CATÃLOGO GLOBAL E IA
// ================================================================

// CARREGAR TABELA DO CATÃLOGO
async function carregarCatalogoGlobal() {
    ...
    } catch (e) { console.error("Erro ao carregar catÃ¡logo:", e); }
}

// BUSCA DE FOTO VIA IA (Gemini 2.5 Flash)
async function iaBuscarFoto() {
    const nome = document.getElementById('cat-nome').value;
    const btn = window.event.target; // Captura o botÃ£o clicado
    ...
    try {
        ...
        if (data.success) {
            ...
            alert("A IA nÃ£o encontrou uma foto exata. Tente ser mais especÃ­fico no nome.");
        }
    } catch (e) {
        alert("Erro ao consultar serviÃ§o de IA.");
    } finally {
...
async function salvarProdutoGlobal() {
    ...
    if (!payload.nome) return alert("O nome do produto Ã© obrigatÃ³rio.");
    ...
    try {
        ...
        if (data.success) {
            ...
            alert("Produto salvo com sucesso no CatÃ¡logo Global!");
        } else {
...
// ================================================================
// MÃ“DULO 4: GESTÃƒO DE PREÃ‡OS (VÃNCULO ISOLADO)
// ================================================================
...
async function vincularAoMercado(produtoId) {
    const preco = prompt("PreÃ§o para este mercado:", "0.00");
...
// ================================================================
// UTILITÃRIOS E INICIALIZAÃ‡ÃƒO
// ================================================================
...
// ================================================================
// 2. MÃ“DULO CATÃLOGO (PRODUTOS GLOBAIS)
// ================================================================

// FUNÃ‡ÃƒO PARA ABRIR O MODAL (O que estava faltando)
...
function abrirModalProdutoGlobal() {
    ...
    if (!modalElement) {
        console.error("Erro: O elemento modalProdutoGlobal nÃ£o existe no HTML!");
        return;
    }
...
// LÃ³gica para Novo Mercado com GeolocalizaÃ§Ã£o AutomÃ¡tica
async function salvarNovoMercado() {
    const payload = {
        nome: document.getElementById('m-nome').value,
        endereco: document.getElementById('m-endereco').value
...
async function salvarProdutoGlobal() {
    ...
    try {
        ...
        if (data.success) {
            ...
            // Atualiza a tabela do catÃ¡logo para mostrar o novo item
            carregarCatalogoGlobal();
        } else {
...
// 2. Novas funÃ§Ãµes para Gerenciamento de UsuÃ¡rios
...
async function editarPontosUsuario(id, pontosAtuais) {
    const novosPontos = prompt("Digite a nova pontuaÃ§Ã£o para este usuÃ¡rio:", pontosAtuais);
...
// --- FUNÃ‡ÃƒO DE BUSCA EM TEMPO REAL ---
function filtrarUsuarios() {
    // Pega o termo digitado e transforma em minÃºsculo para comparar
    const termo = document.getElementById('input-busca-usuarios').value.toLowerCase();
    // Pega todas as linhas da tabela de usuÃ¡rios
    ...
    linhas.forEach(linha => {
        ...
        // Se o termo estiver no texto, mostra a linha, senÃ£o esconde
...
async function enviarAviso() {
    ...
    if (!payload.titulo || !payload.mensagem) return alert("Preencha tÃ­tulo e mensagem!");
...
async function identificarEAN() {
    ...
    if (ean.length < 8) return; // Ignora cÃ³digos curtos
    ...
    try {
        ...
        if (data.success && data.product) {
            ...
            if (data.exists) {
                console.log("Produto jÃ¡ existente no banco carregado.");
            } else {
...
async function verificarStatusBanco() {
    ...
    if (!badge) return; // Sai da funÃ§Ã£o se o elemento nÃ£o existir
    ...
    try {
        ...
        badge.innerHTML = 'Erro de ConexÃ£o';
    }
}
